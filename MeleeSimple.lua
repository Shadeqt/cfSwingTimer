function cfSwingTimer.initMelee()
	if not cfSwingTimerDB[cfSwingTimer.MODULES.MAIN_HAND].enabled then return end
	...
	local offHandEnabled = db[cfSwingTimer.MODULES.OFF_HAND].enabled
	local extraAttacks = 0

	-- Frame + bars
	local frame = CreateFrame("Frame", "cfSwingTimerFrame", UIParent)
	frame:SetPoint("CENTER", db.mainHand.x, db.mainHand.y)
	frame:SetSize(db.mainHand.width, db.mainHand.height)
	cfSwingTimer.MakeMovable(frame, "mainHand")

	local mainHandBar = cfSwingTimer.CreateSwingBar(frame, 2)
	mainHandBar:SetPoint("TOP")
	local mhColor = db.mainHand.color
	mainHandBar:SetStatusBarColor(mhColor.r, mhColor.g, mhColor.b)
	if not db.mainHand.showBorder then mainHandBar.border:Hide() end
	cfSwingTimer.ApplyBarSettings(mainHandBar, "mainHand")

	local offHandBar
	if offHandEnabled then
		local ohFrame = CreateFrame("Frame", "cfSwingTimerOHFrame", UIParent)
		ohFrame:SetPoint("CENTER", db.offHand.x, db.offHand.y)
		ohFrame:SetSize(db.offHand.width, db.offHand.height)
		cfSwingTimer.MakeMovable(ohFrame, "offHand")

		offHandBar = cfSwingTimer.CreateSwingBar(ohFrame)
		offHandBar:SetPoint("TOP")
		local ohColor = db.offHand.color
		offHandBar:SetStatusBarColor(ohColor.r, ohColor.g, ohColor.b)
		if not db.offHand.showBorder then offHandBar.border:Hide() end
		cfSwingTimer.ApplyBarSettings(offHandBar, "offHand")
		offHandBar:Hide()
	end
	...
	cfSwingTimer.bars["mainHand"] = mainHandBar
	if offHandBar then cfSwingTimer.bars["offHand"] = offHandBar end
